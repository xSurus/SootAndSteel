...
public class KeyboardInputProvider(Keys up, Keys down, Keys left, Keys right, Keys action) : AbstractInputProvider
{
    ...
    public override bool IsActionJustPressed() => currentKeyboardState.IsKeyDown(action) && previousKeyboardState.IsKeyUp(action);
    
    public override bool IsActionHeld() => currentKeyboardState.IsKeyDown(action);

    public override bool IsStartJustPressed() => currentKeyboardState.IsKeyDown(Keys.Enter) && previousKeyboardState.IsKeyUp(Keys.Enter);
}